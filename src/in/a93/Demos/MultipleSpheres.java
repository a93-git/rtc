...
public class MultipleSpheres {
	public static void drawWorld() {
		...
		floor.setTransform(floor.getTransform().scale(10, 0.1f, 10));
		...
		left_wall.setTransform(left_wall.getTransform().scale(10, 0.01f, 10).rotateY((float) (-1 * Math.PI / 4)).rotateX((float) Math.PI / 2).translate(0, 0, 5));
		...
		right.setTransform(right.getTransform().scale(0.5f, 0.5f, 0.5f).translate(1.5f, 0.6f, -0.5f));
		...
		left.setTransform(left.getTransform().scale(0.33f, 0.33f, 0.33f).translate(-1.5f, 0.4f, -0.75f));
		...
		world.setObject(0, floor);
		world.setObject(1, left_wall);
